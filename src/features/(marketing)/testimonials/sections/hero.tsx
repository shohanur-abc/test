import { Star } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Section } from '@/components/section';

// ============= MAIN COMPONENT =============
export default function Hero({ badge, title, highlight, description, rating, totalReviews }: IHero) {
    return (
        <Section className="space-y-8 text-center">
            <BadgeLabel text={badge} />
            <Title text={title} highlight={highlight} />
            <Description text={description} />
            <RatingSummary rating={rating} totalReviews={totalReviews} />
        </Section>
    );
}

// ============= CHILD COMPONENTS =============
const BadgeLabel = ({ text }: { text: string }) => (
    <div className="flex justify-center">
        <Badge variant="secondary" className="rounded-full px-4 py-1 text-sm font-medium">
            {text}
        </Badge>
    </div>
);

const Title = ({ text, highlight }: { text: string; highlight?: string }) => (
    <h1 className="text-4xl @sm:text-5xl @lg:text-6xl @3xl:text-7xl font-bold tracking-tight leading-tight max-w-4xl mx-auto">
        {text}{' '}
        {highlight && <span className="text-primary">{highlight}</span>}
    </h1>
);

const Description = ({ text }: { text: string }) => (
    <p className="text-lg @sm:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
        {text}
    </p>
);

const RatingSummary = ({ rating, totalReviews }: { rating: IRatingSummary; totalReviews: string }) => (
    <div className="flex flex-col @sm:flex-row items-center justify-center gap-4 pt-4">
        <div className="flex items-center gap-2">
            <StarRating value={rating.value} />
            <span className="text-2xl font-bold text-foreground">{rating.value}</span>
            <span className="text-sm text-muted-foreground">/ {rating.max}</span>
        </div>
        <span className="hidden @sm:block text-muted-foreground">•</span>
        <p className="text-sm text-muted-foreground">
            Based on <span className="font-semibold text-foreground">{totalReviews}</span> reviews
        </p>
    </div>
);

const StarRating = ({ value }: { value: number }) => (
    <div className="flex gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
            <Star
                key={i}
                className={`size-5 ${i < Math.round(value) ? 'fill-yellow-400 text-yellow-400' : 'fill-muted text-muted'}`}
            />
        ))}
    </div>
);

// ============= TYPES =============
interface IRatingSummary {
    value: number;
    max: number;
}

interface IHero {
    badge: string;
    title: string;
    highlight?: string;
    description: string;
    rating: IRatingSummary;
    totalReviews: string;
}
