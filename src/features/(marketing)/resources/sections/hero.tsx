...
// ============= MAIN COMPONENT =============
export default function Hero({ badge, title, highlight, description, searchPlaceholder, categories }: IHero) {
    return (
        <Section className="space-y-8 text-center">
            ...
            {categories && <CategoryFilters categories={categories} />}
        </Section>
    );
}
...
const CategoryFilters = ({ categories }: { categories: ICategoryFilter[] }) => (
    <div className="flex flex-wrap items-center justify-center gap-2">
        {categories.map((category, i) => (
            <Badge
                key={i}
                variant={category.active ? 'default' : 'outline'}
                className="rounded-full px-4 py-1.5 text-sm cursor-pointer hover:bg-primary hover:text-primary-foreground transition-colors"
            >
                {category.label}
